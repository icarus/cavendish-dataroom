...
function ValorizacionesChart() {
  return (
    <figure className="my-8 border border-[#E5E7EB] rounded-lg p-6 bg-white shadow-sm">
      <p className="font-sans font-medium text-[#111111] text-base mb-6">
        ...
        {bars.map((bar) => {
          ...
          return (
            <div key={bar.label} className="flex flex-col items-center justify-end flex-1 h-full">
              ...
              <span
                className="font-mono font-medium text-[#111111] mb-1 leading-tight text-center"
              ...
              {/* bar */}
              <div
                className={`w-full rounded-t-sm ${bar.platanus ? "bg-[#FFEC40]" : "bg-[#111111]"}`}
                style={{ height: barH }}
              />
            </div>
          );
      ...
      {/* x-axis line */}
      <div className="border-t border-[#111111] mt-0" />
      {/* x labels */}
      <div className="flex gap-3 mt-2">
        {bars.map((bar) => (
          <div key={bar.label} className="flex-1 text-center">
            <span
              className="font-mono font-medium text-[#111111] leading-tight text-center block"
              style={{ fontSize: "clamp(8px, 1vw, 11px)" }}
            >
              {bar.label}
            </span>
      ...
      <figcaption className="font-sans font-medium text-[#6B7280] text-sm leading-relaxed mt-4">
        Carta y data propia.
      </figcaption>
    </figure>
  );
...
function cellColor(val: number): string {
  if (val >= 60) return "bg-[#FFEC40] text-[#111111] font-medium";
  if (val >= 30) return "bg-[#FEF9C3] text-[#111111]";
  if (val >= 10) return "bg-[#F3F4F6] text-[#111111]";
  ...
  return (
    <figure className="my-8">
      <table className="w-full border-collapse table-fixed">
        <thead>
          <tr>
            <th className="font-mono font-medium text-[#111111] text-xs uppercase tracking-wider text-left p-1.5 border border-[#E5E7EB] bg-white w-[18%]">
              Entrada / Exit
            </th>
            {exits.map((e) => (
              <th key={e} className="font-mono font-medium text-[#111111] text-xs text-center p-1.5 border border-[#E5E7EB] bg-white">
                ${e}M
              </th>
            ))}
          </tr>
        ...
        <tbody>
          {entries.map((entry, ri) => (
            <tr key={entry.label}>
              <td className={`font-mono font-medium text-xs p-1.5 border border-[#E5E7EB] ${entry.platanus ? "bg-[#111111] text-[#FFEC40]" : "bg-white text-[#111111]"}`}>
                {entry.label}
                {entry.platanus && <span className="block text-[#FFEC40]/60" style={{ fontSize: "10px" }}>Platanus</span>}
              </td>
              {retornosData[ri].map((val, ci) => (
                <td key={ci} className={`font-mono font-medium text-xs text-center p-1.5 border border-[#E5E7EB] ${cellColor(val)}`}>
                  {val}x
                </td>
              ))}
            </tr>
...
export function SectionEstrategia() {
  return (
    <section id="estrategia" className="py-16 border-b border-[#E5E7EB]">
      <span className="inline-block bg-[#FFEC40] text-black font-mono font-medium text-base px-2 py-0.5 rounded mb-4">04</span>
        ...
        <table className="w-full border-collapse">
          <thead>
            <tr className="border-b border-[#E5E7EB]">
              <th className="text-left font-mono font-medium text-[#111111] text-base uppercase tracking-wider pb-3 pr-8">Retorno</th>
              <th className="text-left font-mono font-medium text-[#111111] text-base uppercase tracking-wider pb-3">% del portafolio</th>
            </tr>
          </thead>
          <tbody>
            {[
              ...
              <tr key={row.range} className="border-b border-[#F3F4F6]">
                <td className="font-sans font-medium text-base py-3 pr-8">
                  {row.highlight ? <span className="bg-[#FFEC40] px-1 rounded">{row.range}</span> : <span className="text-[#111111]">{row.range}</span>}
                </td>
                <td className="font-sans font-medium text-base py-3">
                  {row.highlight ? <span className="bg-[#FFEC40] px-1 rounded">{row.pct}</span> : <span className="text-[#111111]">{row.pct}</span>}
                </td>
              </tr>
            ))}
          </tbody>
      ...
      <Callout>
        <p className="font-sans font-medium text-[#111111] text-base leading-relaxed">
          En los mejores fondos casi un <span className="bg-[#FFEC40] px-1 rounded">95% de los retornos</span> se compone de ese <span className="bg-[#FFEC40] px-1 rounded">10% del portafolio</span> que retorna en promedio <span className="bg-[#FFEC40] px-1 rounded">60x</span>. Sin esas startups ganadoras no se llega al 3x tan anhelado.
        </p>
      </Callout>
      ...
      <div className="space-y-4 mb-6">
        ...
        <ol className="list-decimal list-outside pl-5 space-y-2">
          <li className="font-sans font-medium text-[#111111] text-base leading-relaxed">¿Es posible en general Latam que las startups retornen múltiplos de 20x, 50x o 100x?</li>
          <li className="font-sans font-medium text-[#111111] text-base leading-relaxed">¿Pueden nuestros fondos llegar a las características mostradas por los portafolios de fondos exitosos?</li>
        </ol>
        <p className="font-sans font-medium text-[#111111] text-base leading-relaxed">
        ...
        <p className="font-sans font-medium text-[#111111] text-base leading-relaxed">
          Ejemplo. Para Platanus Cavendish, con una inversión de $200k, un fund returner debe retornar <span className="bg-[#FFEC40] px-1 rounded text-[#111111]">75x</span> la inversión inicial.
        </p>
        <p className="font-sans font-medium text-[#111111] text-base leading-relaxed">
      ...
      <Callout>
        <p className="font-sans font-medium text-[#111111] text-base leading-relaxed">
          Salvo nuestra valorización estándar de <span className="bg-[#FFEC40] px-1 rounded">$200k por un 7%</span>, ninguna otra inversión logra llegar a un múltiplo de retorno de 60x.
        </p>
      </Callout>
      ...
      <Callout>
        ...
        <p className="font-sans font-medium text-[#111111] text-base leading-relaxed mt-3">
          Si suponemos que en 3–4 años levantará una ronda a 2.5x de valorización y consideramos una dilución de 25%, el potencial exit sería de <span className="bg-[#FFEC40] px-1 rounded">$17,226,562 — un fund returner de 86x</span>.
        </p>
      </Callout>
      ...
      <div className="space-y-20">
        ...
        <div className="flex gap-6">
          <span className="inline-flex items-start justify-center bg-[#FFEC40] text-black font-mono font-medium text-base w-8 h-7 rounded shrink-0 mt-0.5">01</span>
          <div>
            <h4 className="font-sans font-medium text-[#6B7280] text-base mb-3">Nos aseguramos que haya un tech en el equipo</h4>
            <div className="space-y-3">
              ...
              <p className="font-sans font-medium text-[#111111] text-base leading-relaxed">Ventajas de tener un cofundador técnico:</p>
              <ol className="list-decimal list-outside pl-5 space-y-2">
                {[
                  ...
                  <li key={item.title} className="font-sans font-medium text-[#111111] text-base leading-relaxed">
                    <span className="text-[#111111]">{item.title}:</span> {item.body}
                  </li>
                ))}
              </ol>
            </div>
        ...
        {/* Mitigación 2 */}
        <div className="flex gap-6">
          <span className="inline-flex items-start justify-center bg-[#FFEC40] text-black font-mono font-medium text-base w-8 h-7 rounded shrink-0 mt-0.5">02</span>
          <div>
            <h4 className="font-sans font-medium text-[#6B7280] text-base mb-3">Somos buenos identificando equipos</h4>
            <div className="space-y-3">
              <p className="font-sans font-medium text-[#111111] text-base leading-relaxed">
              ...
              <p className="font-sans font-medium text-[#111111] text-base leading-relaxed">
                Desde el 2020 hemos revisado <span className="bg-[#FFEC40] px-1 rounded">9.549 postulaciones</span>, conducido <span className="bg-[#FFEC40] px-1 rounded">1.697 entrevistas</span> e invertido en <span className="bg-[#FFEC40] px-1 rounded">121 startups</span>. Semestre a semestre hemos ido desarrollando el músculo de ir identificando características que aumentan las posibilidades de que un equipo logre armar una startup exitosa.
              </p>
            </div>
          </div>
        </div>
        ...
        {/* Mitigación 3 */}
        <div className="flex gap-6">
          <span className="inline-flex items-start justify-center bg-[#FFEC40] text-black font-mono font-medium text-base w-8 h-7 rounded shrink-0 mt-0.5">03</span>
          <div>
            <h4 className="font-sans font-medium text-[#6B7280] text-base mb-3">Portafolios más grandes</h4>
            <div className="space-y-3">
              <p className="font-sans font-medium text-[#111111] text-base leading-relaxed">
              ...
              <p className="font-sans font-medium text-[#111111] text-base leading-relaxed">
                Tenemos más posibilidades de estar en 2 o 3 startups excepcionales entre <span className="bg-[#FFEC40] px-1 rounded">40–45 inversiones</span> que entre 10.
              </p>
              <p className="font-sans font-medium text-[#111111] text-base leading-relaxed">
        ...
        {/* Mitigación 4 */}
        <div className="flex gap-6">
          <span className="inline-flex items-start justify-center bg-[#FFEC40] text-black font-mono font-medium text-base w-8 h-7 rounded shrink-0 mt-0.5">04</span>
          <div>
            <h4 className="font-sans font-medium text-[#6B7280] text-base mb-3">Aumentar las posibilidades de éxito</h4>
            <div className="space-y-3">
              <p className="font-sans font-medium text-[#111111] text-base leading-relaxed">
              ...
              <p className="font-sans font-medium text-[#111111] text-base leading-relaxed">Lo principal para dar ese valor agregado es:</p>
              <ul className="list-disc list-outside pl-5 space-y-2">
                {[
