...
export function SectionPortafolio() {
  return (
    <section id="portafolio" className="py-16 border-b border-white/10">
      <span className="inline-block bg-[#FFEC40] text-black font-mono font-medium text-base px-2 py-0.5mb-4">05</span>
      ...
      <div className="space-y-4 mb-10">
        ...
        <p className="font-sans font-medium text-white/70 text-sm leading-relaxed">Para esto, hay que fijar algunos supuestos:</p>
        ...
        <table className="w-full border-collapse">
          <tbody>
            {[
              ...
              <tr key={row.label} className={`border-b ${row.bold ? "border-white/10 bg-[#FFEC40]" : "border-white/10"}`}>
                <td className={`font-sans font-medium text-sm py-2.5 pr-8 ${row.bold ? "text-white" : row.sub ? "text-white pl-4" : "text-white"}`}>
                  {row.label}
                </td>
                <td className={`font-mono font-medium text-sm py-2.5 text-right ${row.bold ? "text-white" : "text-white"}`}>
                  {row.value}
                </td>
              </tr>
            ))}
      ...
      <div className="space-y-4 mb-6">
        {[
          ...
          <div key={type.name} className="border border-white/10 p-5 bg-white/5">
            <h4 className="font-mono font-medium text-white/40 text-sm uppercase tracking-wider mb-2">{type.name}</h4>
            <p className="font-sans font-medium text-white/70 text-sm leading-relaxed mb-3">{type.desc}</p>
            <ul className="list-disc list-outside pl-5 space-y-2">
              {type.details.map((d) => (
                <li key={d} className="font-sans font-medium text-white/70 text-sm leading-relaxed">{d}</li>
              ))}
            </ul>
          </div>
        ))}
      ...
      <p className="font-sans font-medium text-white/70 text-sm leading-relaxed mb-4">Realizaremos tres tipos de inversiones:</p>
      <div className="space-y-3 mb-10">
        {[
          ...
          <div key={item.title} className="flex gap-4 py-3 border-b border-white/10 last:border-0">
            <span className="text-[#FFEC40] bg-[#111111] w-5 h-5 flex items-center justify-center shrink-0 mt-0.5 text-xs">→</span>
            <div>
              <span className="font-sans font-medium text-white/70 text-sm">{item.title}: </span>
              <span className="font-sans font-medium text-white/70 text-sm leading-relaxed">{item.body}</span>
            </div>
          </div>
        ))}
      </div>
            ...
            <tbody>
              {[
                ...
                <tr key={row.label} className="border-b border-white/10">
                  <td className="font-sans font-medium text-white/70 text-sm py-2.5 pr-4">{row.label}</td>
                  <td className="font-mono font-medium text-white/70 text-sm py-2.5 text-right">{row.value}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <p className="font-sans font-medium text-white/70 text-sm leading-relaxed mb-10">
      ...
      {/* Portafolio esperado */}
      <h2 className="font-sans font-medium text-white mb-8 mt-4" style={{ fontSize: "clamp(16px, 2vw, 22px)", lineHeight: 1.2 }}>
      ...
      <div className="space-y-4 mb-6">
        ...
        <p className="font-sans font-medium text-white/70 text-sm leading-relaxed">Este sería el avance del portafolio según las tasas de éxito:</p>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            ...
            <div key={item.label} className="border border-white/10 p-4 bg-white/5 text-center">
              <p className="font-mono font-medium text-white text-2xl mb-1">{item.value}</p>
              <p className="font-sans font-medium text-white/70 text-sm leading-snug">{item.label}</p>
            </div>
          ))}
        </div>
      </div>
          ...
          <thead>
            <tr className="border-b border-white/10">
              {["MOIC", "% startups", "# startups", "Val. exit", "Exit value", "Total"].map((h) => (
                <th key={h} className="text-left font-mono font-medium text-white text-xs uppercase tracking-wider pb-3 pr-3">{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {[
              ...
              <tr key={row.moic} className={`border-b ${row.highlight ? "bg-[#FFEC40]" : "border-white/10"}`}>
                <td className="font-mono font-medium text-white/70 text-sm py-2.5 pr-3">{row.moic}</td>
                <td className="font-sans font-medium text-white/70 text-sm py-2.5 pr-3">{row.pct}</td>
                <td className="font-mono font-medium text-white/70 text-sm py-2.5 pr-3">{row.n}</td>
                <td className="font-mono font-medium text-white/70 text-sm py-2.5 pr-3">{row.val}</td>
                <td className="font-mono font-medium text-white/70 text-sm py-2.5 pr-3">{row.exit}</td>
                <td className="font-mono font-medium text-white/70 text-sm py-2.5">{row.total}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <div className="space-y-4 mt-6 mb-10">
        <h4 className="font-mono font-medium text-white/40 text-sm uppercase tracking-wider">Generaciones a la fecha</h4>
        <p className="font-sans font-medium text-white/70 text-sm leading-relaxed">
          A la fecha el fondo ya ha realizado 4 inversiones a través de dos generaciones:
        </p>
        <ul className="list-disc list-outside pl-5 space-y-2">
          <li className="font-sans font-medium text-white/70 text-sm leading-relaxed">Generación 25: 4 startups.</li>
        </ul>
        <p className="font-sans font-medium text-white/70 text-sm leading-relaxed">
          Estamos en proceso de selección de la generación 26-I, donde ya hemos seleccionado 4 startups.
        </p>

        <h4 className="font-mono font-medium text-white/40 text-sm uppercase tracking-wider mt-4">Proyección de startups por generación</h4>
        <p className="font-sans font-medium text-white/70 text-sm leading-relaxed">Para llegar a las 32 startups, invertiremos en 6 generaciones:</p>
        <div className="grid grid-cols-3 sm:grid-cols-6 gap-3">
          {[
            ...
            <div key={g.gen} className="border border-white/10 p-3 bg-white/5 text-center">
              <p className="font-mono font-medium text-white text-xl mb-0.5">{g.n}</p>
              <p className="font-mono font-medium text-white text-xs uppercase tracking-wider">Gen {g.gen}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="space-y-4 mb-6">
        ...
        <p className="font-sans font-medium text-white/70 text-sm leading-relaxed">Las inversiones oportunísticas están destinadas a:</p>
        <ul className="list-disc list-outside pl-5 space-y-2">
          <li className="font-sans font-medium text-white/70 text-sm leading-relaxed">High Pre Seed que estén levantando a una valorización que haga difícil aceptar el ticket estándar (por ejemplo están levantando una ronda de $750k a una valorización de $5M y ya llevan $300k levantados). Como son inversiones en teoría con mayor tasa de éxito, sacrificamos un porcentaje de participación para bajar el riesgo al portafolio.</li>
          <li className="font-sans font-medium text-white/70 text-sm leading-relaxed">Fundadores Low Pre Seed que nos gustaría invertirles menos que los $200k del deal fijo del programa de aceleración.</li>
        </ul>
        <p className="font-sans font-medium text-white/70 text-sm leading-relaxed">
          A la fecha ya hemos realizado dos: <span className="text-white">Boom</span> y <span className="text-white">Utanix</span>.
        </p>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
      ...
      <div className="overflow-x-auto">
        <table className="w-full border-collapse">
          <thead>
            <tr className="border-b border-white/10">
              <th className="text-left font-mono font-medium text-white text-sm uppercase tracking-wider pb-3 pr-8">Tipo de ticket</th>
              <th className="text-right font-mono font-medium text-white text-sm uppercase tracking-wider pb-3">Total exit value</th>
            </tr>
          </thead>
          <tbody>
            {[
              ...
              <tr key={row.label} className="border-b border-white/10">
                <td className="font-sans font-medium text-white/70 text-sm py-3 pr-8">{row.label}</td>
                <td className="font-mono font-medium text-white/70 text-sm py-3 text-right">{row.value}</td>
              </tr>
            ))}
            <tr className="bg-[#FFEC40]">
              <td className="font-sans font-medium text-white/70 text-sm py-3 pr-8">Total</td>
              <td className="font-mono font-medium text-white/70 text-sm py-3 text-right">$63,714,286</td>
            </tr>
          </tbody>
        </table>
      </div>
