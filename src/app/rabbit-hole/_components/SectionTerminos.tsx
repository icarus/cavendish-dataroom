...
export function SectionTerminos() {
  ...
  return (
    <section id="terminos" className="py-16">
      <span className="inline-block bg-[#FFEC40] text-black font-mono font-medium text-base px-2 py-0.5 rounded mb-4">09</span>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-10">
        {terms.map((t) => (
          <div key={t.label} className="border border-[#E5E7EB] rounded-lg p-5 bg-white shadow-sm">
            <p className="font-mono font-medium text-[#111111] text-base uppercase tracking-wider mb-1">{t.label}</p>
            <p className="font-sans font-medium text-[#111111] text-base">{t.value}</p>
            {t.sub && <p className="font-sans font-medium text-[#111111] text-base mt-0.5">{t.sub}</p>}
          </div>
        ))}
      </div>
        ...
        <table className="w-full border-collapse">
          <thead>
            <tr className="border-b border-[#E5E7EB]">
              <th className="text-left font-mono font-medium text-[#111111] text-base uppercase tracking-wider pb-3 pr-8">Período</th>
              <th className="text-left font-mono font-medium text-[#111111] text-base uppercase tracking-wider pb-3">Fee anual</th>
            </tr>
          </thead>
          <tbody>
            {[
              ...
              <tr key={row.period} className="border-b border-[#F3F4F6]">
                <td className="font-sans font-medium text-[#111111] text-base py-3 pr-8">{row.period}</td>
                <td className="font-mono font-medium text-[#111111] text-base py-3">{row.fee}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <p className="font-sans font-medium text-[#111111] text-base leading-relaxed mb-4">
        Al firmar el Subscription Agreement se paga el <span className="bg-[#FFEC40] px-1 rounded text-[#111111]">30%</span> del capital comprometido. El resto sigue este calendario tentativo:
      </p>
      <Callout>
          ...
          <thead>
            <tr className="border-b border-[#E5E7EB]">
              {["Call", "Fecha", "% del call", "% acumulado"].map((h) => (
                <th key={h} className="text-left font-mono font-medium text-[#111111] text-base uppercase tracking-wider pb-3 pr-8">{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {capitalCalls.map((row) => (
              <tr key={row.call} className="border-b border-[#F3F4F6]">
                <td className="font-mono font-medium text-[#111111] text-base py-2 pr-8">{row.call}</td>
                <td className="font-sans font-medium text-[#111111] text-base py-2 pr-8">{row.date}</td>
                <td className="font-mono font-medium text-[#111111] text-base py-2 pr-8">{row.pct}</td>
                <td className="font-mono font-medium text-[#111111] text-base py-2">{row.total}</td>
              </tr>
            ))}
          </tbody>
        </table>
