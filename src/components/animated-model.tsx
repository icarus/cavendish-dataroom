...
export function AnimatedModel({
  ...
  const getColorFromLightness = useCallback(
    (lightness: number) => {
  ...
  const initializePixelPositions = useCallback(() => {
    ...
    const canvasSize = resolution * (pixelSize * 2.75);

    pixelPositionsRef.current = newPositions.map((pixel) => ({
      ...pixel,
      x: pixel.targetX + (Math.random() - 0.5) * canvasSize * 2,
      y: pixel.targetY + (Math.random() - 0.5) * canvasSize * 2,
      z: 1000,
    }));
  ...
  }, [updatePixelPositions, drawPixels, initializePixelPositions, onAnimationComplete, pixelSize, resolution]);
